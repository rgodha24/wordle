...
def getLetterList(possibleAnswers, yellowWeight, greenWeight, greens):
    letterList = []
    x = []
    for q in range(26):
        x.append(0)
...
def f(x):
    ...
    for wordleAnswerInt in tqdm(range(loopNumber)):
        ...
        while True:
            ...
            letterList = getLetterList(possibleAnswers, yellowWeight, greenWeight, greens)
            ...
            if damn == True:
                ...
                # print(greens, yellows, greys, cantBe, triedList)
    return mean(benchmarkList)


initialGuesses = [1, 45]
res = minimize(f, initialGuesses, options={
    "maxiter": 100,
    "disp": True
})
print(res.x)